...
export function useSSE() {
  ...
  const start = useCallback(async (files, apiBase, authToken) => {
    abortedRef.current = false
    setStage('uploading'); setStatusText('正在上传…')
    setDetail(''); setError(null); setResult(null)
    ...
    try {
      ...
      es.onmessage = (e) => {
        ...
        switch (d.stage) {
          ...
          case 'ocr':
            ...
            break
          case 'stage1':
            if (d.status === 'done') { setStage('stage2'); setStatusText('正在生成精讲…'); setDetail('共' + d.question_count + '题 · ' + (d.exam_type || '')) }
            break
          case 'stage2':
            if (d.status === 'progress') setDetail('AI 正在深度分析中…')
            else if (d.status === 'done') { setStage('done'); setResult(d); es.close(); esRef.current = null }
            break
          case 'error':
            setError({ message: d.message, recoverable: d.recoverable }); setStage('error')
      ...
      es.onerror = () => {
        ...
        if (++sseErrors >= 3) { es.close(); esRef.current = null; if (!abortedRef.current) setError({ message: '连接多次中断，请重试', recoverable: true }); setStage('error') }
      }
