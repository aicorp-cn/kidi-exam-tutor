...
export function HomeScreen() {
  const { goProcessing, config, history, setHistory, histPage, setHistPage, histDone, setHistDone, TYPE_LABEL, VARIANT_LABEL } = useApp()
  ...
  const loadHistory = useCallback(async (reset = false) => {
    const page = reset ? 1 : histPage
    if (reset) { setHistory([]); setHistPage(1); setHistDone(false) }
    if (histDone && !reset) return
    setHistLoading(true)
    try {
      const r = await fetch(config.apiBase + '/exams?page=' + page)
      const { items, total } = await r.json()
      if (!items?.length) { setHistDone(true); setHistLoading(false); return }
      setHistory(prev => reset ? items : [...prev, ...items])
      setHistPage(page + 1)
      if ((page * config.pageSize + items.length) >= total) setHistDone(true)
    } catch { setHistDone(true) }
    setHistLoading(false)
  }, [histPage, histDone, config, setHistory, setHistPage, setHistDone])
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col">
      {/* Hero */}
      <div className="text-center pt-8 pb-2 px-6 shrink-0">
      ...
      {/* Upload zone */}
      <div
        className={`mx-4 p-7 border-2 border-dashed rounded-xl text-center cursor-pointer transition-all shrink-0 bg-exam-bg-alt ${
      ...
      {/* History */}
      <div className="border-t border-exam-border-light mt-5 flex-1 min-h-0 overflow-y-auto px-4 pt-4">
        <h3 className="text-[0.72rem] text-exam-text-muted font-medium uppercase tracking-wider mb-2 sticky top-0 bg-exam-bg pb-2 z-10">最近</h3>
        ...
        {history.map(item => (
          <div key={item.id} className="bg-exam-surface rounded-lg p-3.5 mb-2 cursor-pointer border border-transparent active:border-exam-accent active:bg-exam-surface-hover transition-all"
            onClick={() => {/* navigate to review from history */}}>
        ...
        {!histDone && history.length > 0 && (
          <div className="text-center py-4">
            <button className="text-exam-accent text-sm border border-exam-border px-8 py-2 rounded-full font-medium hover:bg-indigo-400/8 transition-colors disabled:opacity-30" onClick={() => loadHistory(false)} disabled={histLoading}>
              {histLoading ? '加载中…' : '加载更多'}
            </button>
          </div>
        )}
      </div>
