  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-2 shrink-0">
        ...
        <button
          onClick={() => { setManageMode(!manageMode); setSelected(new Set()); setDeleteConfirm(false) }}
          className={`shrink-0 text-xs px-3 py-2 rounded-lg font-medium transition-colors ${
            manageMode ? 'bg-exam-accent/15 text-exam-accent' : 'bg-exam-surface border border-exam-border text-exam-text-muted hover:text-exam-text'
          }`}
        >
          {manageMode ? '完成' : '管理'}
        </button>
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-4">
        ...
        {items.map(item => (
          <div key={item.id}
            className={`bg-exam-surface rounded-lg p-3.5 mb-2 border transition-all ${
              manageMode
                ? selected.has(item.id)
                  ? 'border-exam-accent bg-indigo-400/5 cursor-pointer'
                  : 'border-transparent cursor-pointer'
                : 'border-transparent cursor-pointer active:border-exam-accent active:bg-exam-surface-hover'
            }`}
            ...
            <div className="flex items-center gap-2">
              ...
              {manageMode && (
                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${
                  selected.has(item.id) ? 'bg-exam-accent border-exam-accent text-white' : 'border-exam-border'
                }`}>
              ...
              {/* Star — always visible, non-manage mode */}
              {!manageMode && (
                <button
        ...
        <div className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto px-4 py-3 bg-exam-bg border-t border-exam-border-light">
          {deleteConfirm ? (
            <div className="text-center">
              <p className="text-sm text-exam-text-muted mb-3">
                将删除 <span className="text-exam-error font-semibold">{selected.size}</span> 条记录。此操作不可撤销。
              </p>
              <div className="flex gap-3 justify-center">
                <button
                  onClick={() => setDeleteConfirm(false)}
                  className="px-6 py-2 rounded-full text-sm border border-exam-border text-exam-text-muted hover:text-exam-text transition-colors"
                >取消</button>
                <button
                  onClick={batchDelete}
                  className="px-6 py-2 rounded-full text-sm bg-exam-error text-white font-semibold active:scale-95 transition-transform"
                >确认删除</button>
              </div>
            </div>
          ) : (
            <button
              ...
              className="w-full py-2.5 bg-exam-error text-white rounded-full text-sm font-semibold active:scale-95 transition-transform"
            >
              删除所选 ({selected.size})
            </button>
          )}
